...
public class TweetServiceImpl implements TweetService{
    ...
    public Tweet update(Long id, TweetRequest tweetRequest) {
        ...
        Tweet tweet = tweetRepository.findById(id)
                .orElseThrow(() -> new CustomException("Tweet not found with this id: "+id));

        if(tweet.getUser() == null || !tweet.getUser().getId().equals(userService.getCurrentUser().getId())){
            throw new CustomException("You are not authorized to update this tweet!");
        }
        tweet.setContent(tweetRequest.getContent());
        tweet.setUpdateDate(LocalDate.now());
    ...
    public void delete(Long id) {
        ...
        if(tweet.getUser() == null || !tweet.getUser().getId().equals(userService.getCurrentUser().getId())){
            throw new CustomException("You are not authorized to delete this tweet!");
        }
    ...
    public Tweet findById(Long id) {
        ...
        return tweetRepository.findById(id)
                .orElseThrow(() ->
                        new CustomException("Tweet not found with id: " + id)
                );
    ...
    public List<Tweet> findByUserId(Long userId) {
        ...
        if (!userService.getCurrentUser().getId().equals(userId)) {
            throw new CustomException("You are not authorized to view these tweets!");
        }
